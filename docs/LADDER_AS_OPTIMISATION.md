...
**This is a free certificate that nothing currently claims.** `mtram.load_pre`
re-estimates each series with fue on the way in, so its gate compares the joint
diagonal fit against *re-estimated* univariate likelihoods and the difference is
≈0 whatever it was handed. Computing the likelihood once at the stored values —
one evaluation, no optimisation — would tell an analyst whether the files they
were given are optima or something someone edited. See §7.
...
Note what the gate does **not** prove. It proves the crossing, not that the
`.pre` files were optima — see §2.1. The two are independent, and only the
first is checked today.
...
## 7. What the analysis says should change

The flow is sound and the conventions carry real guarantees. Two gaps, both
small, both following from §2.1:

* **The gate could verify the `.pre` for free and does not.** One likelihood
  evaluation at the stored values, before re-estimating, yields the optimality
  gap of §2.1 — non-negative, zero exactly when the files are optima. Today
  nothing in the suite can tell a certified `.pre` from a file that merely has
  the extension. Reporting the gap would close that, at the cost of one
  evaluation and no optimisation.

* **`.inp` input should be documented as first-class.** `mcp_server.load_pre`
  already accepts one and handles it correctly — it re-estimates, so the seeds
  are only seeds; fed art's `.inp` with every parameter at zero it reaches the
  same likelihoods as with the `.pre` (−1744.135582 both ways). Since by the
  rule an edited `.pre` *is* an `.inp`, this is the normal case after any
  reformulation, and the tool's name suggests otherwise.

Neither is a defect in what the files mean. Both are places where the
implementation knows less than the convention does.
...
someone has to come back and ask why.
